...
│   │   └── KwikAiApiClient.ts             # SSE streaming client for /op4 Kwik AI endpoint
│   │
│   ├── auth/
...
│   │
│   ├── gokwik-auth/
│   │   ├── GokwikAuthManager.ts          # getAuthenticatedContext() — gokwik-profile/ persistent context
...
│   │
│   ├── kwik-ai/
│   │   └── AiResponseParser.ts           # parseReply(markdown) → AdRow[]; handles ₹ + comma formatting
│   │
│   ├── meta-scraper/
...
├── data-validation/
│   └── specs/
│       ├── meta-vs-dashboard.spec.ts    # Spec 1: KwikAds API vs Meta Ads Manager benchmark parity
│       ├── kwikads-toggle.spec.ts       # Spec 4: toggle state read — TOGGLE_ON/OFF/BLOCKED/META_NOT_ONBOARDED
│       ├── kwikads-admin-override.spec.ts # Spec 5: PATCH toggle flip + restore
│       ├── kwikads-storefront-events.spec.ts # Spec 6: sp/op1 + e/op5 firing on homepage + PDP
│       └── kwikads-validator.spec.ts    # Spec 2: 3-step combined validator (disk + storefront + platform)
│
├── kwik-ai/
│   └── specs/
│       ├── kwik-ai-api.spec.ts          # Spec 2: AiResponseParser unit tests (no browser)
│       └── kwik-ai-live.spec.ts         # Spec 3: live /op4 SSE endpoint — parses + prints table
│
├── scripts/
...
│   ├── login-all.ts                     # orchestrates all 4 login scripts with SessionChecker
...
└── validator/
    ...
    └── verdict-assembler.test.ts        # 8 unit tests — covers all 6 verdict states
```
...
|---|---|
| `/pages` (UI layer) | `src/core/meta-scraper/`, `src/core/kwikads-platforms/`, `src/core/storefront/` |
| `/tests` | `src/data-validation/specs/`, `src/kwik-ai/specs/` |
| `/api` | `src/core/api-client/BaseApiClient.ts`, `src/core/api-client/KwikAiApiClient.ts` |
| `/validators` | `src/core/data-engine/DataComparator.ts`, `src/validator/verdict-assembler.ts` |
| `/utils` | `src/core/utils/` |
| `/network` | `src/core/network/network-observer.ts` |
| `/config` | `src/config/env.config.ts`, `playwright.config.ts` |
| `/auth` | `src/core/auth/`, `src/core/gokwik-auth/`, `src/core/gk-admin-auth/`, `src/core/shopify-partner-auth/`, `src/core/meta-scraper/MetaAuthManager.ts` |
